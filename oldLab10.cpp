...
char lights::parsemorse(string)
{
    ...
    /* int i,j;
     ...
     cout<<"thisis the length of finaltranslate: "<< FinalTranslate[1].length()<<endl; // finaltranslate is only getting the length of the first one and nothing else
     cout<<"thisis the length of finaltranslate: "<< FinalTranslate[i].length()<<endl; // prints out the length is 0
     
     for(i=0;i<FinalTranslate->length();i++)
     {
...
/*void lights::showlights()
{
    ...
    *PBDDR &= 0xFFFFFFFE;	// configures port B0 as input (first push button). You could use: &= ~0x01
    ...
    if(character == '-')//yellow find out what number to change 80 to
    {
...

{
    ...
    cout<<"this is the start of somethign new"<<endl<<endl<<endl;
